...
mod6: Ctrl+N 只在 custom model 间切换 (_find_at_or_near 容错, /model 菜单不受影响)
...
custom7: 多行历史记录按↓无法返回空输入框 → 修复
custom8: Welcome 页面全部文字橙色 + 版本号后加 "Modified" 标记
...
select: 1,2,3,4,5,6,7,8,9,custom7,custom8 / all / restore
```
...
| 6   | model cycle  | peek/cycle 函数 | 覆盖H+移除检查  | 0    | Ctrl+N 只切换 custom model (_find_at_or_near 容错) |
...
| 补偿 | 死代码区域   | 多处死代码   | 注释/缩短填充   | -132 | 统一补偿 mod3(+1) + mod9(+66) + custom8(+65) |
| custom7 | 多行历史   | `V(),!0`     | `(spaces)!1`   | 0    | 多行历史按↓可返回空输入框             |
| custom8 | Welcome 橙色 | 5 处 Text 属性 | +`color:"#FFA500"` | +65  | 全部欢迎文字橙色 + 版本号加 Modified |
...
- mod6: 修改 `peekNextCycleModel`, `peekNextCycleSpecModeModel`, `cycleSpecModeModel` 三个函数
  （`cycleModel` 是委托函数，无需修改；使用 _find_at_or_near 容错定位）
...
- custom8: Ij() 中 5 处 Text 元素添加 #FFA500 hex color，版本号追加 " Modified"
...
### Custom 8: Welcome 页面橙色 + "Modified" 标记

**位置**: `Ij` 函数（Welcome 页面渲染）

**修改**: 5 处 Text 元素添加 `color:"#FFA500"` 属性，版本号追加 " Modified"
...
| 提示 | `{children:VAR}` | `{color:"#FFA500",children:VAR}` | +16 |
| 文件夹 | `{dimColor:!0,children:[...]}` | `{color:"#FFA500",children:[...]}` | +4 |

**效果**: 欢迎页面全部文字显示为橙色，版本号后带 "Modified" 标记。

**稳定锚点**: `dimColor:!0,children:"v` + 版本号正则 — 版本字符串唯一，其余元素用距离定位。
...
mods/custom6_custom_model_cycle.py     # Ctrl+N custom model 切换, 容错版 (0 bytes)
mods/custom7_multiline_history.py      # 多行历史↓键修复 (0 bytes)
mods/custom8_welcome_modified.py       # Welcome 橙色 + Modified 标记 (+65 bytes)
```

mod3 产生 +1 byte，mod9 产生 +66 bytes，custom8 产生 +65 bytes，合计 +132 bytes。
由 `comp_universal.py 132` 统一补偿。
...
用法：`python3 comp_universal.py 132` 补偿 mod3(+1) + mod9(+66) + custom8(+65) 的 +132 bytes。
...
- custom6 注释: ~36B — validateModelAccess 注释掉的检查
...
python3 ~/.factory/skills/droid-bin-mod/scripts/mods/custom6_custom_model_cycle.py
...
python3 ~/.factory/skills/droid-bin-mod/scripts/mods/custom8_welcome_modified.py   # +65 bytes
python3 ~/.factory/skills/droid-bin-mod/scripts/compensations/comp_universal.py 132 # 补偿 mod3(+1) + mod9(+66) + custom8(+65)

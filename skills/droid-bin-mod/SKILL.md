...
| 补偿 | 死代码区域   | 多处死代码   | 注释/缩短填充   | ~-136 | 统一补偿 mod3(+1) + mod6(~+24) + mod8(+45) + mod9(+66) |
...
mods/mod6_custom_model_cycle.py  # Ctrl+N 跳过 Copilot model (~+24 bytes)
...
mod3 产生 +1 byte，mod6 产生 ~+24 bytes，mod8 产生 +45 bytes，mod9 产生 +66 bytes，合计 ~+136 bytes。
由 `comp_universal.py` 统一补偿（实际字节数以 mod6 输出为准）。
...
用法：`python3 comp_universal.py N` 补偿所有 mod 的总字节增量（N = mod3 + mod6 + mod8 + mod9 的实际输出之和）。
...
python3 ~/.factory/skills/droid-bin-mod/scripts/compensations/comp_universal.py 136 # 补偿 mod3(+1) + mod6(~+24) + mod8(+45) + mod9(+66)

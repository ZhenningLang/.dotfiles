#!/usr/bin/env python3
"""mod-hide-command-truncation: 截断条件短路，永远返回原文 (0 bytes)"""
import sys
sys.path.insert(0, str(__file__).rsplit('/', 2)[0])
from common import load_droid, save_droid, replace_one, V

NAME = 'mod-hide-command-truncation'
...
assert len(data) == original_size, f"{NAME} 大小变化 {len(data) - original_size:+d} bytes"
